...
check_service "https://sparkify.dev/api/health" "API (public via Cloudflare)" 10 || echo "⚠️  Cloudflare tunnel may still be connecting"
check_service "https://sparkify.dev" "Frontend (public via Cloudflare)" 5 || echo "⚠️  Cloudflare tunnel may still be connecting"
...
echo "   - Public (TLS via Cloudflare): https://$DOMAIN"
echo "   - Origin (HTTP only):          http://localhost"
echo ""
...
echo "   - Managed by Cloudflare at the edge; none required locally"
echo ""
